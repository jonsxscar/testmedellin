...
<body>
    <div class="container">
        ...
        if (empty($requestId)) {
            echo '<div class="message error">Error: No se proporcionó un requestId.</div>';
            exit;
        }
        ...
        try {
            ...
            if (isset($responseData['status']) && isset($responseData['status']['status'])) {
                ...
                switch ($transactionStatus) {
                    case 'APPROVED':
                        echo '<div class="status success">La transacción ha sido aprobada.</div>';
                        break;
                    case 'PENDING':
                        echo '<div class="status pending">La transacción está pendiente.</div>';
                        break;
                    case 'REJECTED':
                        echo '<div class="status failed">La transacción ha sido rechazada.</div>';
                        break;
                    default:
                        echo '<div class="status error">Estado desconocido de la transacción.</div>';
                        break;
                }
            } else {
                echo '<div class="message error">Error: No se encontró información de estado en la respuesta.</div>';
            }
        } catch (Exception $e) {
